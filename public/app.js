...
function renderTaskList() {
    ...
    taskList.innerHTML = items.map((task) => {
        ...
        const previewModes = previewsForTask(task)
            .map((p) => (p.mode === 'ours' ? '已转换' : '官方') + ':' + p.port)
            .join(' · ');
...
function showResultForTask(task) {
    ...
    previewOursBtn.textContent = ours ? `已转换 :${ours.port}` : '预览已转换';
...
function updatePreviewStatus(info) {
    ...
    ${activePreviews.map((p) => `
      <span class="preview-pill">
        <a href="${escapeHtml(p.url)}" target="_blank">${escapeHtml(p.label || (p.mode === 'ours' ? '已转换' : '官方'))} · ${escapeHtml(p.name)}:${p.port}</a>
...
previewOursBtn.addEventListener('click', async () => {
    const task = getTask(selectedTaskId);
    // 「已转换」优先部署包（有 adapter-hosts）；没有则退回 dist，但兼容层可能不生效
    const dir = (task && task.interfaceReplaced && task.migratedPath)
        ? task.migratedPath
        : previewDirForTask(task);
    if (!dir) return;
    // 每次点「预览已转换」都强制清一次前端余额/登录持久化，避免 417.xxx 幽灵余额
    if (task) task.needAuthReset = true;
    if (!(task && task.interfaceReplaced)) {
        appendTaskLog(
            task.id,
            '提示：尚未「替换接口」。预览已转换仍会开兼容层开关；若目录无 adapter-hosts，行为接近静态壳。',
            false
        );
    }
